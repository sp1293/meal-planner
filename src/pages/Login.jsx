...
export function LoginPage({ navigate }) {
  ...
  return (
    <AuthLayout
      title="Welcome back"
      subtitle="Sign in to your NourishAI account"
...
export function SignupPage({ navigate }) {
  ...
  return (
    <AuthLayout
      ...
      subtitle="Start planning better meals today — free forever"
      ...
      {step === 1 && (
        <>
          ...
          <form onSubmit={handleStep1}>
            ...
            <div className="form-group">
              ...
              {password && (
                <div style={{ display: "flex", gap: 4, marginTop: 6 }}>
                  {[1,2,3,4,5].map(i => (
                    <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i <= strength.score ? strength.color : "var(--border)", transition: "var(--transition)" }} />
                  ))}
                </div>
              )}
            </div>
            ...
            <button type="submit" className="btn btn-primary btn-full" style={{ padding: 13, fontSize: 15 }}>
              Continue →
            </button>
          </form>
        </>
      ...
      {step === 2 && (
        <form onSubmit={handleSignup}>
          ...
          <button type="submit" className="btn btn-primary btn-full" disabled={loading || !gender}
            style={{ padding: 13, fontSize: 15, marginBottom: 10 }}>
            {loading ? <><span className="spin">⟳</span> Creating account...</> : "Create My Account 🚀"}
          </button>

          <button type="button" className="btn btn-ghost btn-full btn-sm" onClick={() => { setStep(1); setError(""); }}>
            ← Back
          </button>
